...
window.filterKateg = (x) => {
  filterlenenKitablar.innerHTML = ``;
  const favorites = JSON.parse(localStorage.getItem("favorites")) || [];

  const filteredBooks = books.filter(
    (book) => book.category.trim().toLowerCase() === x.trim().toLowerCase()
  );
...
window.checkStock = () => {
  ...
  // YALNIZ sales dəyəri 20 və aşağı olan kitabları filterləyirik
  const filteredBooks = books.filter((book) => book.sales <= 20);
